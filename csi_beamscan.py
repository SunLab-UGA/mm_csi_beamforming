...
if __name__ == "__main__":
    ...
    # save some parameters to a text file
    logger.info("Saving parameters to a text file")
    filename = f'csi_data_{time_suffix}.txt'
    with open(filename, 'w') as file:
        file.write(f"Time suffix: {time_suffix}\n") # TODO replace with experiment name/short description
        file.write(f"Number of beams: {len(csi_data)/packets_per_beam}\n")
        file.write(f"Number of packets per beam: {packets_per_beam}\n")
        file.write(f"Number of samples: {len(csi_data)}\n")
        file.write(f"theta_step: {theta_step}\n") if theta_step else None
        file.write(f"phi_step: {phi_step}\n") if phi_step else None
        file.write(f"Scan Time taken: {beamscan_time} seconds\n")
        # device information
        file.write(f"UDBox serial number: {udbox.serial_number}\n")
        file.write(f"TX BBoxOne5G serial number: {txbbox.serial_number}\n")
        file.write(f"RX BBoxOne5G serial number: {rxbbox.serial_number}\n")
        # beam gain
        file.write(f"final rx Beam gain: {rxbbox.beam['beam_gain']}\n")
        file.write(f"final tx Beam theta: {txbbox.beam['beam_gain']}\n")
